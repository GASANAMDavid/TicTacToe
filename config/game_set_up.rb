...
module GameSetUp
  def self.setup(choice)
    ...
    case choice
    when 1
      first_player_info = Interface.human_player_info(I18n.t('first'))
      second_player_info = Interface.human_player_info(I18n.t('second'))
    ...
    when 2
      first_player_info = Interface.human_player_info(I18n.t('human'))
      second_player_info = Interface.random_computer_info
      first_player = Human.new(first_player_info[:name], first_player_info[:symbol])
      second_player = RandomComputer.new(second_player_info[:name], second_player_info[:symbol])
    when 3
      first_player_info = Interface.human_player_info('human')
      second_player_info = Interface.intelligent_computer_info
      first_player = Human.new(first_player_info[:name], first_player_info[:symbol])
      second_player = IntelligentComputer.new(second_player_info[:name], second_player_info[:symbol])
    end
    system('clear')
    choice = Interface.choose_who_plays_first(first_player, second_player)
    if choice == 1
